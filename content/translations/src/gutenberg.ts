 ...
 * search gutendex
 */
export async function gutendexSearch(
  query: string,
...
function pickBestFormat(formats: Record<string, string>) {
  ...
  throw new Error(
    "no recognized text/plain or text/html format in gutendex data",
  );
}
...
export async function fetchBookText(bookId: number) {
  console.log(
    `fetchBookText: requesting metadata from gutendex for bookId=${bookId}`,
  );
