...
function instantiateOpenAiClient(opts: TranslateOptions): OpenAI {
  ...
  if (OPENAI_MODELS.includes(model)) {
    ...
    console.log(
      "using openrouter configuration with baseURL https://openrouter.ai/api/v1",
    );
...
async function callChatCompletion(
  ...
  console.log(
    `calling chat completion for model ${model} with ${
  ...
  console.log(
    `chat completion returned content length ${choice.message.content.length}`,
  );
...
async function translateChunkWithRetries(
  ...
  while (attempts < MAX_RETRIES) {
    ...
    try {
      console.log(
        `translating chunk ${chunkIndex}/${chunkCount}, attempt ${attempts}...`,
      );
...
${chunk}`;
      ...
      console.log(
        `chunk ${chunkIndex} translated successfully (length ${result.length})`,
      );
      return result;
    } catch (err) {
      console.warn(
        `error in chunk ${chunkIndex}/${chunkCount} (attempt ${attempts}): ${String(err)}`,
      );
...
export async function translateFullText(
  ...
  console.log(
    `starting translation for "${title}" by ${author} using model ${model}`,
  );
  ...
  for (let i = 0; i < chunks.length; i++) {
    ...
    console.log(
      `translating chunk ${i + 1}/${chunks.length} (size ${chunk.length})`,
    );
