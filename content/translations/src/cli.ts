...
program
  ...
  .description(
    "cli for searching and translating public domain books w meltdown-tier slang",
  );
...
program
  ...
  .action(async (opts: { bookId: string; model: string; notes: string }) => {
    ...
    try {
      ...
      const spinner = ora(
        chalk.cyanBright(`translating using model=${model}...`),
      ).start();
      ...
      fs.writeFileSync(
        path.join(dirPath, "translation.txt"),
        translation,
        "utf-8",
      );
