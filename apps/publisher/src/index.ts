...
import { createPrintCommand } from "./commands/print.js";
...
program
  ...
  .hook("preAction", (thisCommand) => {
    ...
    if (opts.quiet && opts.verbose) {
      console.error(
        chalk.red("Error: Cannot use --quiet and --verbose together"),
      );
      process.exit(1);
    }
...
program
  ...
  .option(
    ...
    "Platforms to publish to (kdp, lulu, ingram)",
    ["kdp", "lulu"],
  )
  ...
  .action(async (bookSlug: string, options) => {
    ...
    try {
      ...
      if (options.dryRun) {
        spinner.info(
          `Dry run mode - would publish ${bookSlug} to: ${options.platform.join(", ")}`,
        );
      } else {
...
program
  ...
  .action(async (bookSlug: string) => {
    const spinner = ora(
      `Checking status for ${chalk.cyan(bookSlug)}...`,
    ).start();
...
// Add print command (Lulu Print API - Direct Fulfillment)
