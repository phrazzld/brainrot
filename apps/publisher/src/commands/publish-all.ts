...
export function createPublishAllCommand(): Command {
  const publishAllCommand = new Command("publish-all")
    ...
    .option("-p, --platforms <platforms...>", "Platforms to publish to", [
      "kdp",
      "lulu",
    ])
...
async function publishToAllPlatforms(
  ...
  try {
    ...
    if (enabledPlatforms.length === 0) {
      ...
      Logger.error(
        'Enable platforms in metadata.yaml under "publishing" section',
      );
      process.exit(1);
    }
    ...
    for (const platform of enabledPlatforms) {
      ...
      try {
        ...
        if (!options.force) {
          ...
          Logger.error(
            "Aborting remaining platforms. Use --force to continue on errors.",
          );
          break;
        }
      }
    ...
    const report = {
      ...
      metadata: {
        ...
        isbn:
          metadata.formats?.ebook?.isbn || metadata.formats?.paperback?.isbn,
      },
    ...
    if (report.summary.successful === report.summary.total) {
      spinner.succeed(
        `Successfully published ${metadata.title} to all platforms!`,
      );
    } else if (report.summary.successful > 0) {
      spinner.warn(
        `Published to ${report.summary.successful}/${report.summary.total} platforms`,
      );
    } else {
...
async function runPreflightChecks(
  ...
  if (metadata.publishing?.lulu) {
    checks.push({
      ...
      message: metadata.formats?.paperback?.isbn
        ? undefined
        : "Missing paperback ISBN",
    });
  }
  ...
  if (metadata.publishing?.kdp) {
    checks.push({
      ...
      message: metadata.formats?.ebook?.price
        ? undefined
        : "No eBook price set (will use default)",
    });
  }
  ...
  if (metadata.publishing?.lulu) {
    checks.push({
      ...
      message: metadata.formats?.paperback?.price
        ? undefined
        : "No paperback price set (will use default)",
    });
  }
  ...
  switch (platform) {
    case "lulu": {
      ...
      await lulu.setPricing(project.id, [
        ...
        {
          currency: "EUR",
          price: Math.round(pricing * 0.85),
          territories: ["EU"],
        },
        {
          currency: "GBP",
          price: Math.round(pricing * 0.75),
          territories: ["GB"],
        },
      ]);
    ...
    case "kdp": {
      ...
      const result = await kdp.publishCompleteBook(
        bookDetails,
        {
          filePath: path.join(generatedDir, "kindle.epub"),
          format: "epub" as const,
        },
        {
          filePath: path.join(generatedDir, "cover.jpg"),
          format: "jpg" as const,
        },
        {
