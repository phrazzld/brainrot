...
export class BlobService {
    ...
    try {
      ...
      if (existingFile && existingFile.size > 0) {
        ...
        const existingChecksum =
          existingFile.contentDisposition?.match(/checksum=([a-f0-9]+)/)?.[1];
  ...
  public async uploadText(
    ...
    return pRetry(
      ...
      },
      {
        ...
        onFailedAttempt: (error) => {
          console.log(
            `Text upload attempt ${error.attemptNumber} failed. ${error.retriesLeft} retries left.`,
          );
        },
      },
    );
   ...
   */
  public getUrlForPath(
    path: string,
    options?: { baseUrl?: string; noCache?: boolean },
  ): string {
