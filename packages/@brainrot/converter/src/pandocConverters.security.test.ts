...
describe("pandocConverters Security Tests", () => {
    ...
    it("should reject metadata fields not in allowlist", async () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining(
          "[SECURITY] Rejected metadata field not in allowlist: malicious",
        ),
      );
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining(
          "[SECURITY] Rejected metadata field not in allowlist: evil-field",
        ),
      );
      expect(consoleWarnSpy).toHaveBeenCalledWith(
        expect.stringContaining(
          "[SECURITY] Skipping unsafe metadata field: malicious",
        ),
      );
  ...
  describe("Error Handling", () => {
    it("should handle pandoc execution errors gracefully", async () => {
      ...
      await expect(markdownToEpub("# Test", {})).rejects.toThrow(
    ...
    it("should handle pandoc non-zero exit codes", async () => {
      ...
      await expect(markdownToEpub("# Test", {})).rejects.toThrow(
        "Pandoc failed with code 1: Pandoc error details",
      );
    });
  });
});
