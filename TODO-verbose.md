...
- [x] Update Vercel to deploy from new monorepo
### Complexity: MEDIUM
### Started: 2025-08-20
### Completed: 2025-08-20

### Context Discovery
- Two Vercel projects exist: web (old) and brainrot (new)
- vercel.json configured with proper monorepo commands
- Deployment via CLI fails due to root directory detection issues
- Manual dashboard configuration required

### Execution Log
- Updated root vercel.json with monorepo build commands
- Attempted CLI deployments from both root and apps/web
- Created comprehensive manual configuration guide
- Documented two migration paths for Vercel dashboard

### Approach Decisions
- Used pnpm filter commands instead of direct next commands
- Set framework to null to avoid auto-detection issues
- Created manual guide since CLI doesn't support all monorepo settings
- Provided both update-in-place and fresh-start options

### Learnings
- Vercel CLI has limitations with monorepo root directory settings
- Manual dashboard configuration still required for complex monorepos
- Two projects created: keep one, configure properly, delete other
- GitHub integration will enable automatic deployments after manual setup
