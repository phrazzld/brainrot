...
const { stripMarkdown } = require('../packages/@brainrot/converter/dist');
const { BlobService } = require('../packages/@brainrot/blob-client/dist');
...
async function convertAndUpload() {
  ...
  try {
    ...
    for (const file of mdFiles) {
      ...
      const result = await blobService.uploadTextFile(blobPath, plainText);
    ...
    try {
      ...
      const result = await blobService.uploadTextFile(introBlobPath, introText);
