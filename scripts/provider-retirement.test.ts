  ...
  it("has no active Vercel compute or Blob authority", () => {
    const violations = activeRoots.flatMap((path) =>
      activeFiles(join(root, path)).flatMap((file) => {
        ...
        return /\bvercel\b|@vercel\/blob|@brainrot\/blob-client|sync:blob|BLOB_READ_WRITE_TOKEN|NEXT_PUBLIC_BLOB_(?:BASE_URL|DEV_URL)|VERCEL_(?:ENV|ORG_ID|PROJECT_ID|TOKEN|URL)|api\.vercel\.com|\.vercel\.app\b/i.test(
          text,
        )
          ? [relative(root, file)]
          : [];
