...
async function getExistingBlobs(
  ...
  try {
    ...
    console.error(
      chalk.yellow("Warning: Could not fetch existing blobs"),
      error,
    );
    return new Map();
  }
...
async function syncBook(slug: string, options: SyncOptions) {
  ...
  try {
    ...
    if (!generatedExists) {
      ...
      console.log(
        chalk.yellow(`Run 'pnpm generate:formats book ${slug}' first`),
      );
      process.exit(1);
    }
    ...
    const progressBar = new cliProgress.SingleBar(
      {
        format:
          "Uploading |{bar}| {percentage}% | {value}/{total} Files | {filename}",
        barCompleteChar: "█",
        barIncompleteChar: "░",
        hideCursor: true,
      },
      ...
      limit(async () => {
        try {
          if (!options.dryRun) {
            ...
            console.log(
              chalk.gray(
                `  Would upload: ${file.blobPath} (${(file.size / 1024).toFixed(1)}KB)`,
              ),
            );
          }
        } catch (error) {
          errors++;
          console.error(
            chalk.red(`  ✗ Failed to upload ${file.blobPath}:`),
            error,
          );
        }
      }),
...
if (!process.env.BLOB_READ_WRITE_TOKEN) {
  console.error(
    chalk.red("Error: BLOB_READ_WRITE_TOKEN not found in environment"),
  );
  console.error(
    chalk.yellow("Please ensure .env.local contains BLOB_READ_WRITE_TOKEN"),
  );
  process.exit(1);
}
